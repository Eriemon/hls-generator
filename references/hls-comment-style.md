...
- Single-line formatting: short `for`/`while`/`if`/`switch` headers, ordinary local declarations, and ordinary assignments stay on one line when the merged statement fits the configured line limit. Function signatures, long template types, initializer lists, and other statements that genuinely exceed the limit may remain multi-line.
