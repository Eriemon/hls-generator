...
def _delegate_env() -> dict[str, str]:
    ...
    返回:
        已补齐禁写 Python 字节码缓存约束的环境变量映射。
    """

    # 复制当前进程环境，避免直接修改全局环境映射。
    dict_env = dict(os.environ)  # 委托子进程环境变量副本

    # 子进程继续沿用禁写 __pycache__ 的运行约束。
    dict_env["PYTHONDONTWRITEBYTECODE"] = "1"  # 委托子进程禁写 Python 字节码缓存
...
def run_delegate(script_path: Path, argv: list[str] | None = None) -> int:
    ...
    completed_process_result: subprocess.CompletedProcess[str] = subprocess.run(  # 委派进程结果
        list_command,  # 当前委托命令参数序列
        check=False,  # 包装层自行处理失败退出码
        env=_delegate_env(),  # 委托子进程沿用禁写字节码环境约束
    )
...
def run_delegate_retrying_transient_fs(
    ...
    *,
    ...
    for int_attempt in range(1, int_attempts + 1):
        ...
        completed_process_result = subprocess.run(  # 当前尝试得到的委派进程结果
            list_command,  # 当前重试轮次复用的委托命令参数序列
            check=False,  # 重试流程统一读取退出码后自行判定
            capture_output=True,  # 先捕获输出，命中可重试场景时再决定是否回放
            env=_delegate_env(),  # 当前重试轮次继续禁写字节码，避免失败重试又回写缓存
            text=True,  # 让 stderr/stdout 以后续字符串判定方式处理
        )
