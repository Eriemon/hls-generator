...
from scripts.python.generation.prompt import render_prompt

# HLS 注释治理模块只生成计划和门禁报告，不直接改写源码。
from scripts.python.hls_quality_gate.readability.rewrite_plan import build_hls_comment_rewrite_plan
from scripts.python.hls_quality_gate.readability.runner import (
...
from scripts.python.config.skill_dependencies import require_skill_dependencies
from scripts.python.generation.spec import normalize_spec, read_spec, write_spec
from scripts.python.config.user_config import resolve_comment_language
...
# HLS 可读性门禁入口的关键字集合保持与功能域 runner 对齐。
...
def run_hls_readability_gate(
    ...
    **dict_options: Any,
    ...
    # 功能域 runner 负责实际 HLS AST 与注释规则检查。
    readability_report = _run_hls_readability_gate(  # HLS 可读性 runner 返回的报告对象
...
# comment_language 统一通过配置层解析，避免 facade 自行解释 auto。
...
# spec 物化负责把路径或字典输入转换为 workflow 可消费的 spec 文件。
