"""requirements 校验、开放问题与风险检查 helper。"""

# 延迟注解求值，避免运行时解析前向引用。
from __future__ import annotations

# Any 覆盖 JSON 风格容器里的混合值类型。
from typing import Any

# pattern 元数据与开放问题补充能力统一来自 patterns.py。
from scripts.python.generation.patterns import (
    canonical_pattern_name,
    pattern_definition,
    pattern_open_questions,
)
...
# requirements 根模块继续提供 AXI-Stream 与 DATAFLOW 相关的稳定枚举。
from scripts.python.generation.requirements import (
...
# requirements 根模块同时提供共享类型别名。
from scripts.python.generation.requirements import (
    IssueList,
    JsonDict,
)

# parse 子模块继续承接 design_requirements 读取与字段缩窄逻辑。
