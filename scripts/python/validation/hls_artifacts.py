...
from scripts.python.hls_quality_gate.hls_ast_guard import validate_hls_ast_guard

# 导入 hls_config.cfg 解析工具，供静态验证阶段读取时钟和配置项。
from scripts.python.validation.hls_cfg import (
    cfg_relative_path_issue,
    clock_period_ns,
    parse_hls_cfg_entries,
)

# 导入 profile 校验器，确保生成 spec 与约定的 HLS profile 一致。
from scripts.python.validation.hls_profile import validate_hls_profile

# 导入报告指标采集器，把综合与实现结果收敛到统一 metrics 中。
from scripts.python.validation.hls_reports import collect_hls_report_metrics

# 导入 Vitis HLS Tcl 渲染器，供本地编译和 readiness 阶段生成执行脚本。
from scripts.python.validation.hls_tcl import render_vitis_hls_tcl
...
from scripts.python.generation.patterns import (
    ...
    required_pattern_headers,
)

# 导入注释语言约束解析器，统一处理 auto 与显式语言设置。
from scripts.python.generation.prompt import require_comment_language
...
from scripts.python.validation.stage_verifier import plan_contract_interface_issues

# 导入 Vitis 规则扫描器，用于补充特定工具链规则违规诊断。
from scripts.python.validation.vitis_rules import scan_vitis_rule_violations

# 导入向量契约工具，校验测试向量 hash 与合同标签是否匹配。
from scripts.python.generation.vectors import (
    VECTOR_HASH_TAG,
    extract_vector_hashes,
    find_vector_contracts,
)
