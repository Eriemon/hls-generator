"""
readable HLS generator 命令行入口，负责解析子命令并路由到对应运行时能力。
...
def main(argv: list[str] | None = None) -> int:
    """
    运行 readable HLS generator 的命令行入口。
    ...
    if str_legacy_error is not None:
        ...
        argparse.ArgumentParser(prog="readable-hls-gen").exit(2, f"error: {str_legacy_error}\n")
...
def _build_parser() -> argparse.ArgumentParser:
    """
    构造完整的 readable HLS generator CLI 解析器。
    ...
    argument_parser_root = argparse.ArgumentParser(  # 先承接根命令帮助和版本参数，再生成一级子命令注册树
        prog="readable-hls-gen",  # argparse 在帮助与报错里展示的 CLI 程序名
        description="AMD-Xilinx/Vitis readable HLS generator CLI.",  # 根命令 --help 顶部展示的 HLS-only 工具说明
    )
    ...
    argument_parser_root.add_argument(
        "--version",
        action="version",
        version=f"readable-hls-gen {__version__}",
    )
...
def _register_deps_parser(
    ...
    argument_parser_deps: argparse.ArgumentParser = (  # 依赖治理入口命令的一级解析器
        subparser_action_root_commands.add_parser(  # 注册依赖治理总入口
            ...
            help="Check or install readable-hls-generator skill dependencies.",  # 帮助列表中说明它管理技能依赖
        )
    )
...
def _cmd_validate(namespace_args: argparse.Namespace) -> int:
    ...
    if (not namespace_args.no_external) and namespace_args.readiness != "static":
        ...
        # 外部验证前只请求 vitis scope，避免 static 之外的本地路径继续受 core 误拦。
        _require_dependencies_for_use(path_request_dir, scopes={"vitis"})
...
def _cmd_run_workflow(namespace_args: argparse.Namespace) -> int:
    ...
    # workflow 默认只要求本地 core 路径；真正推进外部 Vitis 验证时才追加 vitis scope。
    set_dependency_scopes = {"core"}  # 当前 workflow CLI 入口要请求的依赖 scope 集合

    # 非 static 且允许外部工具时，workflow 会进入 Vitis 指导链路。
    if (not namespace_args.no_external) and namespace_args.readiness != "static":

        # 当前 workflow 确实需要 Vitis 指导时再追加 vitis scope。
...
def _require_resolved_comment_language(comment_language: str) -> str:
    ...
    if str_resolved_language is None:
        ...
        raise ValueError(
            ...
            "`python -m scripts.python.cli.readable_hls_generator user-config "
...
def _require_dependencies_for_use(
    ...
    *,
    ...
    raise ValueError(
        ...
        "Run `python -m scripts.python.cli.readable_hls_generator deps request --out <path>` "
        "or inspect the generated dependency request file."
    )

# 模块以 `python -m scripts.python.cli.readable_hls_generator` 方式运行时走统一入口。
